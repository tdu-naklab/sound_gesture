...
class GestureGame:
    ...
    def update(self):
        ...
        for label in range(1, labels):
            ...
            game_screen = cv2.circle(game_screen, (int(center_x), int(center_y)), 1, (0, 0, 255), -1)
...
class Note:
    ...
    def __init__(self, icon_pos, note_init_pos):
        ...
        self.__note_is_active = False

    # ノートを出現させる
    def generate(self):
        # ノートが非アクティブな場合
        if self.__note_is_active is False:
            self.__note_pos = self.__note_init_pos  # ノーツ位置初期化
    ...
    def update(self, game_screen):
        ...
        if self.__note_is_active is True:
            ...
            self.__note_pos += move_vec  # ノーツを移動

            cv2.circle(game_screen, (int(self.__note_pos[0]), int(self.__note_pos[1])), 30, (255, 0, 0), 5)  # ノートを描画

        return game_screen

    # note_is_activeのゲッター
    @property
    def note_is_active(self):
